...
import '../../features/admin/presentation/screens/admin_submissions_screen.dart';
...
// WITHOUT recreating the router (which would reset navigation to splash).
class _AuthRouterNotifier extends ChangeNotifier {
  _AuthRouterNotifier(Ref ref) {
    ref.listen<bool>(hasSessionProvider, (_, __) => notifyListeners());
  }
}
...
final routerProvider = Provider<GoRouter>((ref) {
  ...
  return GoRouter(
    ...
    redirect: (context, state) {
      ...
      final loggingIn = state.matchedLocation.contains('/login') ||
          state.matchedLocation.contains('/register');

      // Protected routes
      final isProtectedRoute =
          state.matchedLocation.contains('/favorites') ||
          state.matchedLocation.contains('/checkins') ||
          (state.matchedLocation.contains('/profile') &&
              state.matchedLocation != '/profile');

      if (!hasSession && isProtectedRoute) return AppRoutes.login;
      if (hasSession && loggingIn) return AppRoutes.map;
    ...
    routes: [
      ...
      GoRoute(
        ...
        builder: (_, __) => const AdminSubmissionsScreen(),
        redirect: (context, state) {
          final user = ref.read(authProvider).user.valueOrNull;
          if (user?.role != 'admin') return AppRoutes.map;
          return null;
        },
      ),
